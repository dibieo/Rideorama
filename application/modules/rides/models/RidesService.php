...
class Rides_Model_RidesService extends Application_Model_Service
{
    ...
    /**
     * Deletes a ride from the airport
     */
    public function deleteRideFromAirport($id){
      
     $this->deleteRide('\Rideorama\Entity\Ridesfromairport', $id);
    }
    
    /**
     * Deletes a ride to the airport
     */
    public function deleteRideToAirport($id){
     
     $this->deleteRide('\Rideorama\Entity\Ridestoairport', $id);   
    }
    
    private function deleteRide($entity, $id){
      
     $query = $this->em->createQuery("DELETE from $entity u where u.id ='$id'");
     $query->execute();
//     
//     $ride = $this->em->find($entity, $id);
//     $this->em->remove($ride);
//     $this->em->flush();
    }
    /**
    ...
    */
    ...
    /**
     * Check to See if the Driver has responded to this seat request
...
//     $variable_address =  ($trip_data['where'] == "toAirport") ? 'pick_up_address' : 'drop_off_address';
     ...
     $airport_id = $this->airport->getAirportByName($airport_name)->id;
      $query = $this->em->createQuery("UPDATE  $entity u SET u.number_of_seats = '$num_seats', 
                              ...
                              u.arrival_time = :arrival_time, u.distance = '$distance',
